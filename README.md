...
| `BACKUP_INCLUDES`  | no       | Whitespace/newline-separated list of patterns to include before excludes. Empty by default. See [Excludes](#excludes). |
...
Everything else under `/source`, including Home Assistant `.storage`, is included by duplicity's default file selection behavior. An explicitly empty `BACKUP_INCLUDES` or `BACKUP_EXCLUDES` disables the corresponding defaults.
